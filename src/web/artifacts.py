...
class ArtifactStore:
    ...
    def publish(self, title: str, html: str, artifact_id: Optional[str] = None) -> dict:
...
def build_artifact_tools(
    ...
    """构建 publish_artifact / list_artifacts 两个工具（TUI 与 Web 共用）。

    - confirm(preview, is_update) -> bool：写前确认钩子（TUI 接 ConfirmScreen）。约定**只在首次发布**
    ...
    async def _publish(args: dict) -> str:
        ...
        if not html.strip():
            return ("publish_artifact 需要 html（**自包含**网页：单文件、内联 CSS/JS、"
                    "不要引用任何外部资源，否则会被 CSP 拦截）。")
        ...
        if confirm is not None and not is_update:
            ...
            if not ok:
                return "用户取消了发布制品。"
        meta = store.publish(title or (artifact_id or ""), html, artifact_id)
        ...
        return (f"已{verb}制品「{meta['title']}」(id={meta['id']}, v{meta['version']})。\n"
                f"查看：{url}\n"
    ...
    async def _list(_args: dict) -> str:
        ...
        lines = [
            f"- {m['title']} (id={m['id']}, v{m['version']}, {m['updated_at']}) "
            f"→ {artifact_url(base_url, m['id'])}"
            for m in items
        ]
        return "制品列表:\n" + "\n".join(lines)
    ...
    return [
        Tool(
            "publish_artifact",
            "把一段自包含 HTML 发布成可分享、随会话实时更新的网页（制品）；写操作，仅 build。"
            "传相同 id 则原地更新、已打开的页面会自动刷新。"
            "适合：带注释的 PR 走查、数据看板、方案对比、交互控件（滑块/开关）、迁移/排查进度清单。",
            {"title": "页面标题", "html": "完整自包含 HTML（内联 CSS/JS，勿引用外部资源）",
             "id": "可选；更新已有制品时传它的 id"},
            _publish, read_only=False,
        ),
        Tool(
            "list_artifacts", "列出已发布的制品（标题/id/版本/链接）",
