...
每次工具调用后，其返回会以 `[工具 X 结果]` 开头回灌给你——**那是工具的输出，不是用户发来的新消息**，
别把它当成用户在给你看东西或已经结束对话；据此决定下一步或给出最终回答。若任务需要写文件/跑流水线，
**务必真正调用对应工具**（edit_file/write_file/dev_isolated/dev_auto…），只读一眼代码不等于完成。
