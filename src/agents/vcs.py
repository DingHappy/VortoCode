...
def open_pr(repo_root, branch: str, title: str, body: str = "", base: str = "main") -> dict:
    ...
    r = subprocess.run(
        ["gh", "pr", "create", "--head", branch, "--base", base, "--title", title, "--body", body or title],
...
def push_and_open_pr(repo_root, branch: str, title: str, body: str = "",
                     base: str = "main", remote: str = "origin") -> dict:
    """先 push 再开 PR；push 失败就不开 PR。返回 {ok, pushed, url, error}。"""
    pushed = push_branch(repo_root, branch, remote)
    if not pushed["ok"]:
        return {"ok": False, "pushed": False, "url": "", "error": "push 失败: " + pushed["output"]}
    pr = open_pr(repo_root, branch, title, body, base)
